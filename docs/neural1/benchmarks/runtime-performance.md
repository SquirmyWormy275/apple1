...
`python -m neural1.benchmark --output /absolute/ssd/report.json` with the
verified `--ssd-root` and `--expected-uuid` arguments measures WozMon transaction throughput and
