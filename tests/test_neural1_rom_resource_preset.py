...
def test_rom_gets_eighteen_short_requests_without_mutating_source_registry():
    ...
    assert spec.generations == 18 and spec.agents_per_cell == 1
...
def test_staged_rom_prompts_cover_each_block_then_only_execute():
    ...
    assert first == family_objective('4k-mind')
    for generation in range(1, 17):
        prompt = family_objective('256-byte-universe', generation)
        address = 0x208 if generation == 1 else 0x200 + 16 * (generation - 1)
        words = 'eight' if generation == 1 else 'sixteen'
        assert f'Write {words} zero bytes at {address:04X} instead.' in prompt
    final = family_objective('256-byte-universe', 17)
    assert '0400.0407\n0400R' in final and 'two Monitor command lines' in final
    with pytest.raises(Neural1Error):
        family_objective('256-byte-universe', 18)
...
def test_staged_rom_actual_campaign_retains_model_bytes_and_exact_validation(tmp_path):
    ...
    class SyntheticStageProvider(FakeProvider):
        ...
        def generate(self, prompt, *, agent_id, seed):
            ...
            if generation == 0:
                text = '0200: A9 41 20 EF FF 4C 1F FF\n0200.0207\n0200R'
            elif generation <= 16:
                count, address = (8, 0x208) if generation == 1 else (16, 0x200 + 16 * (generation - 1))
                text = f'{address:04X}: ' + bytes(count).hex(' ').upper()
            else:
                text = '0200.0207\n0200R'
    ...
    assert result['task'] == 'compact-rom-output-and-return-v5'
