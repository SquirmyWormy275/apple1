...
'display garbage screen
  ...
  USBCheck := INA[31]
  IF USBCheck >> 0
    Serport.startx(RX_Pin, TX_Pin, %0000, BaudRate)       'Start Serial Port, set baud, pins, etc
  ELSE
      Serport.startx(RX_Pin, TX_Pin, %0001, BaudRate)       'Start Serial Port, set baud, pins, etc
